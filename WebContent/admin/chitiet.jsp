			...
			<main>
				<div class="container">
					<div class="d-flex justify-content-between mt-4">
						...
						<h4>Mã hóa đơn: ${hoadon}</h4>
						<form action="chitietdonhang" id="hangform" method="post">
							<button class="btn btn-primary" type="submit" name="xacnhan"
								value="${hoadon}">Xác nhận</button>
						</form>
					</div>
					...
					<table class="table table-striped table-hover">
						...
						<tbody class="table table-striped table-hover">
							<c:forEach var="h" items="${chitiet}">
								<tr>
									<td>${count}</td>
									<td>${h.getMaDienThoai()}</td>
									<td>${h.getTenDienThoai()}</td>
									<td>${h.getGia()}</td>
									<td>${h.getSoLuong()}</td>
